 ...
 * Author: Ahmad Paykli
 *
 * Created on February 7, 2026, 2:15 PM
 */
...
std_ReturnType lcd_8bit_intialize(const chr_lcd_8bit_t *lcd){
    ...
    if(NULL == lcd){
        ...
        ret = lcd_8bit_send_command(lcd, _LCD_8BIT_MODE_2_LINE);
        ret = lcd_8bit_send_command(lcd, _LCD_DDRAM_START); // 0x80 - start address of Data Display Ram.
    }
...
std_ReturnType lcd_8bit_send_char_data_pos(const chr_lcd_8bit_t *lcd, uint8 row, uint8 column, uint8 data){
    std_ReturnType ret = E_OK;
    std_ReturnType l_pin_counter = 0;
    if(NULL == lcd){
        ret = E_NOT_OK;
    }
    else{
...
static std_ReturnType lcd_4bit_set_cursor(const chr_lcd_4bit_t *lcd, uint8 row, uint8 coulmn){
    ...
    if(NULL == lcd){
        ...
        switch(row){
            case ROW1:
                ret = lcd_4bit_send_command(lcd, (_LCD_DDRAM_START + coulmn));
                break;
            case ROW2:
                ret = lcd_4bit_send_command(lcd, (0xC0 + coulmn));
                break;
            case ROW3:
                ret = lcd_4bit_send_command(lcd, (0x94 + coulmn));
                break;
            case ROW4:
                ret = lcd_4bit_send_command(lcd, (0xD4 + coulmn));
                break;
            default : ;
        }
    }
...
static std_ReturnType lcd_8bit_set_cursor(const chr_lcd_8bit_t *lcd, uint8 row, uint8 coulmn){
    ...
    if(NULL == lcd){
        ...
        switch(row){
            case ROW1:
                ret = lcd_8bit_send_command(lcd, (_LCD_DDRAM_START + coulmn));
                break;
            case ROW2:
                ret = lcd_8bit_send_command(lcd, (0xC0 + coulmn));
                break;
            case ROW3:
                ret = lcd_8bit_send_command(lcd, (0x94 + coulmn));
                break;
            case ROW4:
                ret = lcd_8bit_send_command(lcd, (0xD4 + coulmn));
                break;
            default : ;
        }
    }
