 ...
 * Author: Ahmad Paykli
 *
 * Created on February 10, 2026, 7:23 PM
 */
...
std_ReturnType ADC_Init(const adc_conf_t *_adc){
    ...
    if(NULL == _adc){
        ...
        /* Clear Interrupt Flag : ADC interrupt did not occur */
        ADC_InterruptFlagClear();
        /* Configure the priority interrupt */
#if INTERRUPT_PRIORITY_LEVELS_ENABLE == INTERRUPT_FEATURE_ENABLE
        /* Configure Internal interrupt Priority */
        ...
        ADC_SetInterruptHandler(_adc->ADC_Handler_t);
#if INTERRUPT_PRIORITY_LEVELS_ENABLE==(!INTERRUPT_FEATURE_ENABLE) 
